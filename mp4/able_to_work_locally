    ...
    def propagete(self, word):
        for i in range(self.tag_size):
            ...
            mea_p = np.log((measurement_model[cur_tag].setdefault(word, 0) + self.alpha) /
                           (total_tag_word_count[cur_tag] + self.alpha * len(measurement_model[cur_tag])))
            for j in range(self.tag_size):
                ...
                trans_p = np.log((transition_model[cur_prev_tag].setdefault(cur_tag, 0) + self.alpha) /
                                 (trans_tag_sum[cur_prev_tag] + self.alpha * len(transition_model[cur_prev_tag])))
...
def viterbi_p1(train, test):
    ...
    global tags
    tags = []
    global total_tag_word_count
    global trans_tag_sum
    ...
    #Transform the transition model to log probability version
    """
    for tag_trans_dict in transition_model.values():
        #Each tag_trans_dict is a dictionary with keys as possible next tag and values as the current count
        total_next_tag_count = sum(tag_trans_dict.values())
        for tag_key in tag_trans_dict.keys():
            tag_trans_dict[tag_key] = np.log(float(tag_trans_dict[tag_key] / total_next_tag_count))
    """
    ...
    for i in range(len(test)):
        if i % 100 == 0:
            print(i / len(test))
        sentence = test[i]
        cur_trellis = viterbi_trellis(len(prior), len(sentence), prior, 1)
        cur_sentence_pred = cur_trellis.predict(sentence)
        predicts.append(cur_sentence_pred)
    #raise Exception("You must implement me")
    return predicts
